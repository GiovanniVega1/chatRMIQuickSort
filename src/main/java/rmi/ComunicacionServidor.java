...
public class ComunicacionServidor extends UnicastRemoteObject implements ComunicacionServicio {
    ...
    public void connection(String Ip){
        try {
               ...
               // java.rmi.Naming.rebind("rmi://192.168.84.107:9000/ChatService", chatService);
                java.rmi.Naming.rebind("rmi://"+Ip+":1099/ChatService", chatService);
    ...
    public void sendResultsToClients(String results) throws RemoteException {
        for (InteraccionCliente client : clients.values()) {
            client.receiveMessage("Resultados de ordenación: \n" + results);
        }
    }
    
    @Override
    public void sendResultsToServer(String results, String username) throws RemoteException {
        // Manejar los resultados recibidos
        System.out.println("Resultados recibidos del cliente " + username + ":\n" + results);

        // Reenviar los resultados a todos los clientes conectados
        sendResultsToClients(results);
    }    
